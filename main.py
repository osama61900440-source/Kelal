...
# ─── App writable directory (Android safe) ────────────────────────────────────
# On Android, the current dir is not writable. Use the app's data directory.
try:
    from android.storage import app_storage_path          # only exists on Android
    APP_DIR = app_storage_path()
except Exception:
    # Desktop fallback: use the folder of this script
    APP_DIR = os.path.dirname(os.path.abspath(sys.argv[0]))

# ─── SQLite database path (writable on all platforms) ────────────────────────
DB = os.path.join(APP_DIR, "business.db")

# ─── Optional Amharic font ────────────────────────────────────────────────────
FONT = "Roboto"
_font_candidates = [
    os.path.join(APP_DIR, "Nyala.ttf"),
    "Nyala.ttf",
]
...
class HistoryScreen(Screen):
    ...
    def _switch(self, tab):
        self._tab = tab
        # Clear old canvas layers before re-drawing to avoid stacking
        self.t_ship.canvas.before.clear()
        self.t_profit.canvas.before.clear()
        if tab == "ship":
